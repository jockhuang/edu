...
public class MemcachedClient {
    ...
    public boolean delete(String key, Integer hashCode, Date expiry) {
        ...
        if (key == null) {
            log.warn("null value for key passed to delete()");
            return false;
        }
        ...
        try {
            ...
            log.warn("failed to sanitize your key!", e);
            return false;
        }
        ...
        try {
            ...
            if (DELETED.equals(line)) {
                ...
                log.warn("++++ error deleting key: " + key);
                log.warn("++++ server response: " + line);
            }
        } catch (IOException e) {
            ...
            // exception thrown
            log.warn("++++ exception thrown while writing bytes to server on delete");
            log.warn(e.getMessage(), e);

            try {
                sock.trueClose();
            } catch (IOException ioe) {
                log.warn("++++ failed to close socket : " + sock.toString());
            }
    ...
    private boolean set(String cmdname, String key, Object value, Date expiry, Integer hashCode, boolean asString) {
        ...
        if (cmdname == null || cmdname.trim().equals("") || key == null) {
            log.warn("key is null or cmd is null/empty for set()");
            return false;
        }
        ...
        if (NativeHandler.isHandled(value)) {
            ...
            if (asString) {
                ...
                try {
                    ...
                    log.warn("invalid encoding type used: " + defaultEncoding, ue);
                    sock.close();
                    sock = null;
                    return false;
                }
                ...
                try {
                    ...
                    log.warn("Failed to native handle obj", e);
            ...
            try {
                ...
                // we bail
                log.warn("failed to serialize obj", e);
        ...
        // log.warn("key="+key+", val length="+val.length);
        ...
        if (compressEnable && val.length > compressThreshold) {
            ...
            try {
                ...
                log.warn("IOException while compressing stream: " + e.getMessage());
                log.warn("storing data uncompressed");
            }
        }
        ...
        try {
            ...
            if (STORED.equals(line)) {
                ...
                log.warn("++++ error storing data in cache for key: " + key + " -- length: " + val.length);
                log.warn("++++ server response: " + line);
            }
        } catch (IOException e) {
            ...
            // exception thrown
            log.warn("++++ exception thrown while writing bytes to server on set");
            log.warn(e.getMessage(), e);
    ...
    public long getCounter(String key, Integer hashCode) {
        ...
        if (key == null) {
            log.warn("null key for getCounter()");
            return -1;
        }
    ...
    private long incrdecr(String cmdname, String key, long inc, Integer hashCode) {
        ...
        if (key == null) {
            log.warn("null key for incrdecr()");
            return -1;
        }
        ...
        try {
            ...
            if (line.matches("\\d+")) {
                ...
                log.warn("++++ error incr/decr key: " + key);
                log.warn("++++ server response: " + line);
            }
        } catch (IOException e) {
            ...
            // exception thrown
            log.warn("++++ exception thrown while writing bytes to server on incr/decr");
            log.warn(e.getMessage(), e);
        ...
        try {
            ...
            while (true) {
                ...
                if (line.startsWith(VALUE)) {
                    ...
                    // log.warn("++++ key: " + key +"++++ flags: " + flag+",++++ length: " + length);
                    if (log.isDebugEnabled()) {
                        log.debug("++++ key: " + key);
                    ...
                    if ((flag & F_COMPRESSED) == F_COMPRESSED) {
                        try {
                            ...
                            log.warn("++++ IOException thrown while trying to uncompress input stream for key: " + key
                                    + " -- " + e.getMessage());
                    ...
                    if ((flag & F_SERIALIZED) != F_SERIALIZED) {
                        if (primitiveAsString || asString) {
                            ...
                            try {
                                ...
                                log.warn("++++ Exception thrown while trying to deserialize for key: " + key, e);
                        ...
                        try {
                            ...
                            o = null;
                            log.warn("++++ Exception thrown while trying to deserialize for key: " + key + " -- "
                                    + e.getMessage());
                        }
                    }
            ...
            // exception thrown
            log.warn("++++ exception thrown while trying to get object from cache for key: " + key + " -- "
                    + e.getMessage());
    ...
    public Map<String, Object> getMulti(String[] keys, Integer[] hashCodes, boolean asString) {
        ...
        if (keys == null || keys.length == 0) {
            log.warn("missing keys for getMulti()");
            return null;
        }
            ...
            if (line.startsWith(VALUE)) {
                ...
                if ((flag & F_SERIALIZED) != F_SERIALIZED) {
                    ...
                    try {
                        ...
                        /*
                         * Errors de-serializing are to be expected in the case of a long running server that spans
                         * client restarts with updated classes.
                         */
                        // if we have an errorHandler, use its hook
                        if (errorHandler != null) {
                            errorHandler.handleErrorOnGet(this, e, key);
                        ...
                        o = null;
                        log.warn("++++ InvalidClassException thrown while trying to deserialize for key: " + key
                                + " -- " + e.getMessage());
                        ...
                        o = null;
                        log.warn("++++ ClassNotFoundException thrown while trying to deserialize for key: " + key
                                + " -- " + e.getMessage());
                    }
                }
    ...
    public boolean flushAll(String[] servers) {
        ...
        // return false if unable to get SockIO obj
        if (pool == null) {
            log.warn("++++ unable to get SockIOPool instance");
            return false;
        }
        ...
        for (int i = 0; i < servers.length; i++) {
            ...
            if (sock == null) {
                log.warn("++++ unable to get connection to : " + servers[i]);
                success = false;
                if (errorHandler != null) {
            ...
            try {
                ...
                // exception thrown
                log.warn("++++ exception thrown while writing bytes to server on flushAll");
                log.warn(e.getMessage(), e);
     ...
     * 
     * Returns a map keyed on the servername. The value is another map which contains stats with stat name as key and
     * value as value.
     * 
     * @return Stats map
     */
     ...
     * 
     * Returns a map keyed on the servername. The value is another map which contains item stats with
     ...
     * Retrieves stats items for all servers.
     * 
     * Returns a map keyed on the servername. The value is another map which contains slabs stats with slabnumber:field
     ...
     * 
     * Returns a map keyed on the servername. The value is another map which contains cachedump stats with the cachekey
     * as key and byte size and unix timestamp as value.
     * 
    ...
    private Map stats(String[] servers, String command, String lineStart) {
        ...
        if (command == null || command.trim().equals("")) {
            log.warn("++++ invalid / missing command for stats()");
            return null;
        }
        ...
        // if no servers, then return early
        if (servers == null || servers.length <= 0) {
            log.warn("++++ no servers to check stats");
            return null;
        }
            ...
            try {
                ...
                while (true) {
                    ...
                    if (line.startsWith(lineStart)) {
                        ...
                        log.warn("++++ failed to query stats");
                        log.warn("++++ server response: " + line);
                        break;
                    }
                ...
                // exception thrown
                log.warn("++++ exception thrown while writing bytes to server on stats");
                log.warn(e.getMessage(), e);
    ...
    protected final class NIOLoader {
            ...
            try {
                ...
                while (numConns > 0 && timeRemaining > 0) {
                    ...
                    if (n > 0) {
                        ...
                        log.warn("selector timed out waiting for activity");
                    }
        ...
        private void handleError(Throwable e, String[] keys) {
            ...
            // exception thrown
            log.warn("++++ exception thrown while getting from cache on getMulti");
