 ...
 *     int socketConnectTO = 1000 * 3; // 3 seconds to block on initial connections. If 0, then will use blocking connect
 *                                     // (default)
...
public class SockIOPool {
    ...
    private boolean failback = true; // only used if failover is also set ... controls putting a dead server back into
                                     // rotation
    ...
    public void initialize() {
        ...
        synchronized (this) {
            ...
            if (initialized && (buckets != null || consistentBuckets != null) && (availPool != null)
                    && (busyPool != null)) {
    ...
    public SockIO getSock(String key, Integer hashCode) {
        ...
        if ((this.hashingAlg == CONSISTENT_HASH && consistentBuckets.size() == 1)
            ...
            SockIO sock = (this.hashingAlg == CONSISTENT_HASH) ? getConnection(consistentBuckets.get(consistentBuckets
                    .firstKey())) : getConnection(buckets.get(0));
