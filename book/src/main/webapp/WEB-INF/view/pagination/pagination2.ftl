<#macro pageLine pageUtil queryCondition>
   ...
   <q>到</q>
        <input id="goPage" class="pageframe" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" value="${pageUtil.currentPage!1}" type="text">
    <q>页</q>
    <input onclick="goPage();" id="btn" class="pagebtn" type="button" style="cursor:pointer;">
    </div>
    <span class="page_num">共<em><label id="main_dpVoteList_lblTotalPage">${pageUtil.totalCount}</label></em>条，当前第${pageUtil.currentPage}/${pageUtil.totalPage}页，每页${pageUtil.pageSize}条</span>
