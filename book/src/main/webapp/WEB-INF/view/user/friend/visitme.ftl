...
<head>
<title>谁来看过我_${loginUser.displayName?default(loginUser.userName)}的书房</title>
...
<#include "nav.ftl" />
	<dd class="BookFriend">
		<div class="other">
			<span class="info" >${count} 人来看过您</span>
		</div>
		<dl class="listIsilo">
			<#if (list?? && list?size > 0)>
			<#list list as u >
			<dd>
				...
				<a class="red" style="cursor: pointer;" >写书信</a><br />
			</dd>
			</#list>
			<form id="p" method="get" action="visitme.action" >
...
<script type="text/javascript">
	$(document).ready(
		function(){
			...
			$(".tit a:contains('谁来看过我')").attr('class' , 'hot');
		}
	)
</script>
<#include "../bottom.ftl" />
