...
<head>
<title>我关注的_${loginUser.displayName?default(loginUser.userName)}的书房</title>
...
<#include "nav.ftl" />
	<dd class="BookFriend">
		<div class="other">
			<span class="info" >您关注了 ${count} 个书友</span>
		</div>
		<dl class="listIsilo">
			<#if (list?? && list?size > 0)>
			<#list list as u >
			<dd>
				...
				<a class="red" name="lbtn" rel="${u.userId}" >写书信</a><br />
				<a name="dbtn" rel="${u.userId}" >删除</a>
			</dd>
			</#list>
			<form id="p" method="get" action="meconcern.action" >
...
<script type="text/javascript">
	$(document).ready(
		function(){
			...
			$('a[name="dbtn"]').css({cursor : "pointer"}).bind('click' , function(){
				deleteConcern(this.rel , flushPage);
			})
			$(".tit a:contains('我关注的')").attr('class' , 'hot');
		}
	)
</script>
<#include "../bottom.ftl" />
