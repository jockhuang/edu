...
<div class="crumbs">
	<a href="">首页</a> &gt; <a href="">图书管理</a> &gt; 本机构图书
</div>
<!--内容区-->
<div class="context">
	...
	<form id="updateForm" action="/manage/orgbook/setOrgtreeBooksState.action" method="post">
		...
		<table width="100%" cellspacing="2" cellpadding="8" class="listTable">
			...
			<tr <#if (item_index%2==0)>class="bg1"</#if>>
				<th>
				<input type="checkbox" name="orgBookIds" value="${item.bookId}">
				</th>
				<td class="listbooks1"><a href="/manage/orgbook/showBookDetail.action?id=${item.id}" class="book"><img src="http://${imgDomainName!'img3.chineseall.cn'}${ImageUtils.getBookImgUrl(item.bookId)}"></a><h4><a href="/manage/orgbook/showBookDetail.action?id=${item.id}">${item.bookName!''}</a></h4>
				<p>
...
$("#exportButton").click(function(){
	window.location.href="/manage/orgbook/exportOrgBook.action?${queryCondition}";
	alert("ok");
});
</script>